...
z_tx_list = [8, 40, 80.0, 150.0, 200.0, 250.0, 350.0]
z_tx_list = np.flip(z_tx_list)
freq = 0.35
for j in range(len(z_tx_list)):
    ...
    for i in range(len(year_list)):
        ...
        year2 = year_list[i]
        z_tx = z_tx_list[j]
        print(z_tx, year1, month1, year2, month2)
        ...
        ratio_label = 'Ratio of amplitude $|\psi_2| / |\psi_1| $ [dB] '
        rel_err_label = 'Relative Error [u] $||\psi_2| - |\psi_1|| / |\psi_{1}| $ '
        abs_err_label = 'Absolute Error [u] $||\psi_2| - |\psi_1||$'

        path2plots = 'plots-deep/'
        mid_path = '_f=' + str(int(freq*1e3)) + 'MHz_Ztx=' + str(round(z_tx,2)) + 'm_'
        amp_prefix = path2plots + 'amplitude' + mid_path

        show_bool0 = False
        make_dB_plot(sim1, fc=freq, z_tx=z_tx, label=label1, vmin=None, vmax=None,
        ...
        compare_fields(sim=sim1, sim2=sim2,
                       label0= abs_err_label, label1=label1, label2=label2,
                       fc=freq, z_tx=z_tx,
