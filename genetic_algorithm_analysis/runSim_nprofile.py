...
from data import create_transmitter_array, bscan
'''
I need this function to run a Bscan from a n_profile_list
'''

#TODO: Add Gene and Individual Numbers!!1

if len(sys.argv) != 4:
    print('error! you must enter argument: \npython ' + sys.argv[0] + ' <fname_reference.h6> <fname_nprofile_matrix.h5 i')

fname_reference = sys.argv[1] # comparison file, I also use this as the config file
fname_n_matrix = sys.argv[2] # I use this to store the results
...
for i in range(nDepths):
    for j in range(nRX_x):
        for k in range(nRX_z):
            Corr += fitness_correlation(abs(bscan_ref.bscan_sig[i,j,k]), abs(bscan_npy[i,j,k]))
S_corr = 1/Corr
print(Corr, S_corr)
n_matrix_hdf = h5py.File(fname_n_matrix,'r+')
